...
model_order = ["B$_{\\text{S}}$", "B$_{\\text{U}}$", "IGR", "FM", "PGD"]
...
def highlight_best(df):
    for explainer in explainers_map.values():
        ...
        sup = df.loc[explainer, "B$_{\\text{S}}$"]
        for model in model_order:
            if (model == "B$_{\\text{U}}$") or (model == "B$_{\\text{S}}$"):
                continue
            ...
            for metric in df.columns:
                if metric[1] == "Empty $\\downarrow$":
                    ...
                    if expl[metric].split("$\pm$")[0] < sup[metric].split("$\pm$")[0]:
                        expl[metric] = f"\\underline{{{expl[metric]}}}"
                else:
                    if expl[metric].split("$\pm$")[0] > uns[metric].split("$\pm$")[0]:
                        expl[metric] = f"\\textbf{{{expl[metric]}}}"
                    if expl[metric].split("$\pm$")[0] > sup[metric].split("$\pm$")[0]:
