...
new_folder_path = Path("data/processed/mimiciv_icd10cm")
# create folder
new_folder_path.mkdir(parents=True, exist_ok=True)

data = pl.read_parquet("data/processed/mimiciv.parquet")
splits = pl.read_ipc("data/splits/mimiciv_icd10cm_splits.feather")
data = data.join(splits, on="_id")

# remove not used columns
data = data.drop(
    ["note_seq", "charttime", "storetime", "procedure_codes", "procedure_code_type"]
)

# only keep ICD-10-CM codes
data = data.filter(pl.col("diagnosis_code_type") == "icd10cm")

# remove rare codes
data = remove_rare_codes(data, ["diagnosis_codes"], 10)

# drop duplicates
data = data.unique(subset=["_id"])

# filter out rows with no codes
data = data.filter(pl.col("diagnosis_codes").is_not_null())
