...
def get_grad_attention_callable(
    ...
    **kwargs,
    ...
    def grad_attention_callable(
        ...
        #     attributions = input_embeddings*attn_gradient
        ...
        #     class_attributions[:, idx] = (
        #         attributions.detach().cpu() * label_attentions[:, target]
        #     )
...
def get_deepliftshap_callable(
    ...
    **kwargs,
    ...
    Returns:
        Explainer: Callable DeepLift explainer
    """
    explainer = captum.attr.LayerDeepLiftShap(model, model.roberta_encoder.embeddings)

    def deepliftshap_callable(
        input_ids: torch.Tensor,
        device: str | torch.device,
        target_ids: torch.Tensor,
    ) -> torch.Tensor:
...
def get_lrp_callable(
    model: PLMICD,
    **kwargs,
) -> Explainer:
    """Get a callable DeepLift Shap explainer
    ...
    Returns:
        Explainer: Callable Integrated Gradients explainer
    """
    for module in model.roberta_encoder.modules():
        if isinstance(module, torch.nn.Embedding):
            module.rule = EpsilonRule()

        if isinstance(module, torch.nn.LayerNorm):
            module.rule = IdentityRule()
    e_model = deepcopy(model)
    embedding = e_model.roberta_encoder.embeddings
    del e_model.roberta_encoder.embeddings
    explainer = captum.attr.LRP(e_model)
    chunk_size = e_model.chunk_size
    ...
    def lrp_callable(
        ...
        input_ids = input_ids.to(device)
        chunked_input_ids = e_model.split_input_into_chunks(input_ids, PAD_ID).view(
            -1, chunk_size
        )
        embeddings = embedding(chunked_input_ids).requires_grad_()

        attention_mask = create_attention_mask(input_ids)
        chunked_attention_masks = e_model.split_input_into_chunks(attention_mask, 0)
        ...
        for idx, target in enumerate(target_ids):
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
                attributions = explainer.attribute(
                    embeddings,
                    additional_forward_args=(chunked_attention_masks, True),
                    target=target,
                    verbose=True,
                )
            attributions = reshape_plm_icd_attributions(attributions, input_ids)
...
def diet(
    ...
    while not mask_converged:
        input_embeddings = chunked_embeddings * mask + baseline_embeddings * (1 - mask)
        ...
        if target_id is None:
            ...
            loss = torch.norm(mask, p=1) / (
                ...
                y_probs[:, target_id] - original_y_probs[:, target_id].detach(), p=1
            ) / logits.size(0)
        print(
            torch.norm(
                y_probs[:, target_id] - original_y_probs[:, target_id].detach(), p=1
    ...
    **kwargs,
    ...
    def diet_callable(
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
            ...
            mask = mask.view(input_ids.size(0), -1, 1)[:, : input_ids.size(1)].squeeze(
                -1
            )
            mask = mask / mask.sum()
            print(mask.mean())
            class_attributions[:, idx] = mask.detach().cpu()
        return class_attributions

    return diet_callable
